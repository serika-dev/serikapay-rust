...
pub fn generate_signature(payload: impl AsRef<[u8]>, secret: &str) -> String {
    let mut mac = HmacSha256::new_from_slice(secret.as_bytes()).expect("HMAC takes keys of any size");
...
pub fn verify_signature(payload: impl AsRef<[u8]>, signature: &str, secret: &str) -> bool {
    let Some(hex_sig) = signature.trim().strip_prefix("sha256=") else { return false };
...
pub fn construct_event(payload: impl AsRef<[u8]>, signature: &str, secret: &str) -> Result<Event, Error> {
